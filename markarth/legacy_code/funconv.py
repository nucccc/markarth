...
class FuncData:
    '''
    FuncData shall fetch all the data from a function which will be then
    used for convertion and regeneration
    '''
    ...
    def __init__(
        ...
        self.codelines : list[str] = codelines
        self.statements : list[ast.AST] = self.func_ast.body
    ...
    def _collect_return_type(self) -> str | None:
        ...
        #return_type = self.conversion_options.py2cy_typemap().get(return_type, None)
        return return_type
...
class FuncConverter:
    ...
    def __init__(self, func_ast : ast.FunctionDef, codelines : list[str], conversion_options : ConvOpts = default_convopts()):
        self._func_data : FuncData = FuncData(func_ast, codelines)
        self.conversion_options = conversion_options

    @property
    def func_ast(self):
        return self._func_data.func_ast
    
    @property
    def codelines(self):
        return self._func_data.codelines
    
    @property
    def statements(self):
        return self._func_data.statements

    @property
    def indent_pattern(self):
        return self._func_data.indent_pattern
    
    @property
    def arg_types(self):
        return self._func_data.arg_types
    
    @property
    def return_type(self):
        return self._func_data.return_type

    def convert(self) -> str:
        '''
        convert shall actually execute the app conversion
        '''
        self._func_data._collect_start_attributes()
    ...
    def _regen_code(self) -> str:
        ...
        new_codelines = [self.func_decl] + self.cdef_lines + self._func_data.codelines[self.func_ast.body[0].lineno:self.func_ast.body[-1].end_lineno+1]
        return '\n'.join(new_codelines)
    ...
    def _collect_types(self):
        ...
        # the actual c types
        self.collected_ctypes = py2cy_dict_store(self.collected_ptypes, self.conversion_options.py2cy_typemap())
        ...
        self.cdef_lines = cdef_lines_from_tg(
            ...
            indent_level= 1,
            indent_pattern=self._func_data.indent_pattern
        )
    ...
    def _regen_def_line(self) -> str:
        ...
        if self.return_type is not None:
            ...
            first_line.insert(1, self._func_data.return_type)
            new_def_line = ' '.join(first_line)
        return new_def_line
    
    def _add_ctypes_to_args(self) -> list[str]:
        line_inc = dict()
        arg_types_tg = DictTypeStore(self._func_data.arg_types)
        arg_types_tg = py2cy_dict_store(arg_types_tg, self.conversion_options.py2cy_typemap())
        for arg in self.func_ast.args.args:
            argname = arg.arg
            ...
            if argtype is not None:
                ...
                self._func_data.codelines[arg.lineno] = line
    ...
    def _get_func_decl(self) -> str:
        return '\n'.join(self._func_data.codelines[self.func_ast.lineno:self.func_ast.body[0].lineno])
